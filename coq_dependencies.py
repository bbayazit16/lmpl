        ...
        for name, sig_or_src, body, qualids, notation in iter_sentences(sercomp_args, coq_project_path, relative_file_path):
            ...
            if signature_exists:
                ...
                if src.startswith('Require Import'):
                    ...
                    # From AAA Require Import YYY, ZZZ.

                    # parts = src.split()
                    # base_prefix = parts[1]

                    # resolved_module = module_resolver.coq_module_to_path(
                    #     base_prefix
                    # )

                    # modules = [m.rstrip(',') for m in parts[4:]]
                    # for module in modules:
                    #     print(module, '==>', resolved_module)
                    #     known_qualids_to_file_paths[module] = str(resolved_module)
                    # module is like: Abc.Def, Ghi.Jkl, Def.Mno.
                    # (with the last one having an extra dot)

                    # Then we must watch the importe modules
                    # parts = src.split()
                    # # From Data.Tuple Require Import Tuple.fst, Hello.snd.
                    # # attempt to resolve Data.Tuple.Tuple.fst, etc.
                    # # Watch for last identifiers
                    # base_prefix = parts[1]
                    # modules = [m.rstrip(',') for m in parts[4:]]
                    # # modules are like: Abc.Def, Ghi.Jkl, Def.Mno.
                    # # (with the last one having an extra dot)
                    # i = 0
                    # for module in (f"{base_prefix}.{mod}" for mod in modules):
                    #     parts = module.split('.')
                    #     # Handle the last dot - if we're at the last
                    #     last_ident = parts[-2] if i == len(modules) - 1 else parts[-1]
                    #     require_watches[last_ident] = require_import
