...
# # filename = 'Monad.v'
# relative_file_path = Path(f'GHC/Base.v')
# coq_project_path = Path(
#     '/home/baris/coq-autoform/hs-to-coq/base-thy/_CoqProject'
# )

# # So both the directory to _CoqProject OR the _CoqProject file can be passed as an argument.
# if coq_project_path.is_dir():
#     coq_project_path = coq_project_path / '_CoqProject'

# # dot = run_coqdep(coq_project_path)
# # graph = coq_files_graph_from_dotfile(dot)
# # print(graph.dependency_ordering())
...
# # graph = coq_files_graph_from_dotfile(dot)
# # rm(dot)
# # print(graph.dependency_ordering('files'))

# dpd = run_coq_dpdgraph(coqtop_args, coq_project_path, relative_file_path)
# dot = dpd_to_dot(dpd)
# dot_to_svg(dot)

# coq_graph = coq_signature_graph_from_dotfile(dot)
# deps = coq_graph.dependency_ordering()
# print(deps)
# # print()
# # print(deps)
# # print()

# for name, sig, body, sig_file_deps in iter_sentences(sercomp_args, coq_project_path, relative_file_path):
#     module_deps[name] = sig_file_deps
#     if name != '':
#         sigs[name] = sig
#         if body != '':
#             bodies[name] = body

# print(f"\n{len(sigs)} top-level identifiers collected\n")
# print(f"{len(bodies)} top-level bodies collected\n")

# for name in sigs:
#     print(f"== {name} ==")
#     print(f"is_contained={name in deps}")
#     print("--- signature ---")
#     print(sigs[name])
#     # if bodies[name]:
#     #     print("--- body ---")
...
#         qualids = module_deps[name]
#         for qualid in qualids:
#             abs_dep_path = coq_module_to_path(
#                 qualid, qmap, coq_project_path.parent)
#             print(f"  {qualid} -> {abs_dep_path}")
#         if not qualids:
